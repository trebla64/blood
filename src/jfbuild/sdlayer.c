...
static char gamma_saved = 0;
//static char gamma_supported = 0;
...
// input
char inputdevices=0;
char keystatus[256], keyfifo[KEYFIFOSIZ];
uint8_t keyfifoplc, keyfifoend;
...
int initinput(void)
{
	...
	for (i=0; i<SDLK_LAST; i++) {
		...
		Bstrncpy((char *)keynames[ keytranslation[i] ], SDL_GetKeyName(i), sizeof(keynames[i])-1);
	}
...
int setvideomode(int x, int y, int c, int fs)
{
	...
	UNREFERENCED_PARAMETER(modenum);
	UNREFERENCED_PARAMETER(warnonce);
...
//	copybuf(curpalettefaded, pal, 256);
	for (i = 0; i < 256; i++) {
		pal[i].r = curpalettefaded[i].r;
		pal[i].g = curpalettefaded[i].g;
		pal[i].b = curpalettefaded[i].b;
	}
...
struct sdlappicon sdlappicon = {0};	// Lol, this is the least of my problems
#if 0
static SDL_Surface * loadappicon(void)
{
	SDL_Surface *surf;
