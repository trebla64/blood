...
static int32_t kzipopen(char *filnam)
{
	unsigned int i;
...
void allocache(intptr_t *newhandle, int32_t newbytes, char *newlockptr)
{
	...
	for(z=cacnum-1;z>=0;z--)
	{
		...
		for(i=o1,zz=z;i<o2;i+=cac[zz++].leng)
		{
			...
			uint8_t idx = *cac[zz].lock;
			daval += mulscale32(cac[zz].leng+65536,lockrecip[idx]);
	...
	for(k=numgroupfiles-1;k>=0;k--)
	{
		...
		if (groupfil[k] >= 0)
		{
			for(i=gnumfiles[k]-1;i>=0;i--)
			{
				...
				{
					if (!filename[j]) break;
					if (toupperlookup[(uint8_t)filename[j]] != toupperlookup[(uint8_t)gfileptr[j]])
						{ bad = 1; break; }
				}
				if (bad) continue;
		...
		for (j=i-1;j>=0;j--) {
			...
			for (k=gnumfiles[i]-1;k>=0;k--) {
				...
				for (l=gnumfiles[j]-1;l>=0;l--) {
					...
					bad=0;
					for (m=0;m<13;m++)
						if (toupperlookup[(uint8_t)gfileptr1[m]] != toupperlookup[(uint8_t)gfileptr2[m]]) {
							bad = 1;
							break;
						}
