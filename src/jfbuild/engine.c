...
#include "../screen.h"	// For scrLoadPalette
#include "../tile.h"	// For various tile management
#include "../db.h"
#include "../debug.h"	// For dprintf
#include "../engine_access.h"

// Below are the replaced functions that the engine can call directly
extern int animateoffs(short tilenum, short fakevar);

// Disable some warnings on MSVC
#ifdef _MSC_VER
#	pragma warning(disable : 4101)	// Unreferenced local variable
#	pragma warning(disable : 4244)	// Loss of data on truncation etc.
...
#define MAXYSIZ 256
#define MAXZSIZ 255
//#define MAXVOXMIPS 5	// Moved to build.h
...
int32_t dommxoverlay = 1, beforedrawrooms = 1;
...
#ifdef POLYMOST
static int32_t hitallsprites = 0;
#endif
...
int hicsetsubsttex(int32_t picnum, int32_t palnum, char *filen, float alphacut) { return 0; }
int hicsetskybox(int32_t picnum, int32_t palnum, char *faces[6]) { return 0; }
int hicclearsubst(int32_t picnum, int32_t palnum) { return 0; }
...
static void scansector(short sectnum)
{
	walltype *wal, *wal2;
	spritetype *spr;
	int32_t xs, ys, x1, y1, x2, y2, xp1, yp1, xp2=0, yp2=0, templong;
...
//
static void maskwallscan(int32_t x1, int32_t x2, short *uwal, short *dwal, int32_t *swal, int32_t *lwal)
{
	int32_t x, xnice, ynice;
	intptr_t startx, p, fpalookup;
	int32_t y1ve[4], y2ve[4], tsizx, tsizy;
	char bad;		// Still required in MSVC
	int32_t i, u4, d4, dax, z;	// Still required

	UNREFERENCED_PARAMETER(bad);
	UNREFERENCED_PARAMETER(i);
	UNREFERENCED_PARAMETER(u4);
	UNREFERENCED_PARAMETER(d4);
	UNREFERENCED_PARAMETER(dax);
...
//
// [Replaced for Blood, see replace.cpp]
#if 0
static int32_t animateoffs(short tilenum, short fakevar)
{
	int32_t i, k, offs;
...
// owallmost (internal)
//
static int32_t owallmost(short *mostbuf, int32_t w, int32_t z)
{
...
// wallmost (internal)
//
static int32_t wallmost(short *mostbuf, int32_t w, int32_t sectnum, char dastat)
{
...
// wallscan (internal)
//
static void wallscan(int32_t x1, int32_t x2, short *uwal, short *dwal, int32_t *swal, int32_t *lwal)
{
	int32_t x, xnice, ynice;
	intptr_t fpalookup;
	int32_t y1ve[4], y2ve[4], tsizx, tsizy;
...
static void transmaskvline(int32_t x)
{
	...
	intptr_t palookupoffs, bufplc, p;
	short y1v, y2v;
...
static void drawsprite(int32_t snum)
{
	...
	int32_t tilenum, spritenum;
	char swapped, daclip;

//	long *longptr, vtilenum = 0;
...
//
	//JBF 20031206: Thanks to Ken's hunting, s/(rx1|ry1|rx2|ry2)/n\1/ in this function
static void dorotatesprite(int32_t sx, int32_t sy, int32_t z, short a, short picnum, signed char dashade, uint8_t dapalnum, char dastat, int32_t cx1, int32_t cy1, int32_t cx2, int32_t cy2, int32_t uniqid)
{
	...
	char bad;
	int32_t u4, d4, xx, xend, obuffermode, qlinemode=0, y1ve[4], y2ve[4], ny1, ny2;

	UNREFERENCED_ARRAY(y1ve);
	UNREFERENCED_ARRAY(y2ve);
	UNREFERENCED_PARAMETER(qlinemode);
	UNREFERENCED_PARAMETER(ny1);
	UNREFERENCED_PARAMETER(ny2);
	UNREFERENCED_PARAMETER(bad);
	UNREFERENCED_PARAMETER(u4);
	UNREFERENCED_PARAMETER(d4);
	UNREFERENCED_PARAMETER(xx);
	UNREFERENCED_PARAMETER(xend);
	UNREFERENCED_PARAMETER(obuffermode);
...
//	int32_t fil;
	...
	if (tablesloaded == 0)
	{
		...
		fp = fopen("TABLES.DAT", "rb");
		if (fp != NULL)
//		if ((fil = kopen4load("tables.dat",0)) != -1)
		{
//			kread(fil,sintable,2048*2);
			fread(sintable, 2048*2, 1, fp);
			for (i=2048-1; i>=0; i--) sintable[i] = B_LITTLE16(sintable[i]);
//			kread(fil,radarang,640*2);
			fread(radarang, 640*2, 1, fp);
			for (i=640-1;  i>=0; i--) radarang[i] = B_LITTLE16(radarang[i]);
...
//	int32_t i;
	...
	// Blood overrides this function, so we do it too
	scrLoadPalette();
	return;
...
//
static int32_t getclosestcol(int32_t r, int32_t g, int32_t b)
{
...
//
static int32_t insertspritesect(short sectnum)
{
	short blanktouse;
...
//
static int32_t insertspritestat(short statnum)
{
	short blanktouse;
...
//
static int32_t deletespritesect(short deleteme)
{
...
//
static int32_t deletespritestat(short deleteme)
{
...
{
	int32_t res = (davis >> 8) + dashade;
	if (res < 0)  res = 0;
	if (res > 63) res = 63;
	return res;
	return scrGetPalookup(davis, dashade);
...
#else
int32_t loadmaphack(char *filename) { return -1; }
#endif
...
//		{ printOSD("Nice try.\n"); exit(0); }
	...
	{
		if (lookupsalloctype == 0) kkfree((void *)lookups);
		if (lookupsalloctype == 1) suckcache((int32_t *)lookups);
		lookups = NULL;
	}
	lookupsalloctype = 0;
	...
	horizlookup = (intptr_t *)(lookups);
	horizlookup2 = (intptr_t *)(FP_OFF(lookups)+j);
	horizycent = ((ydim*4)>>1);
...
void nextpage(void)
{
	...
	if (((uint32_t)(mdtims-omdtims)) > 10000) omdtims = mdtims;
#endif
...
int32_t allocatepermanenttile(short tilenume, int32_t xsiz, int32_t ysiz)
{
	...
	assert(0);
	tileAllocTile(tilenume, xsiz, ysiz);
...
{
	return InsertSprite(sectnum, statnum);
...
{
	return ChangeSpriteSect(spritenum, newsectnum);
...
{
	return ChangeSpriteStat(spritenum, newstatnum);
...
void setpalettefade(char r, char g, char b, char offset)
{
	...
	for (i=0;i<256;i++) {
        	tempbuf[k++] =
			(curpalettefaded[i].b =
			   curpalette[i].b + ( ( ( (int32_t)palfadergb.b - (int32_t)curpalette[i].b ) * (int32_t)offset ) >> 6 ) ) >> 2;
        	tempbuf[k++] =
			(curpalettefaded[i].g =
			   curpalette[i].g + ( ( ( (int32_t)palfadergb.g - (int32_t)curpalette[i].g ) * (int32_t)offset ) >> 6 ) ) >> 2;
        	tempbuf[k++] =
			(curpalettefaded[i].r =
			   curpalette[i].r + ( ( ( (int32_t)palfadergb.r - (int32_t)curpalette[i].r ) * (int32_t)offset ) >> 6 ) ) >> 2;
...
#if 1
	intptr_t p;
	int32_t /*p, */xp, yp, xpbpl, ypbpl, d, de, dse, patc=0;
...
{
	int32_t i, xp1, yp1, xp2=0, yp2, tempy;
...
{
	walltype *wal;
	int32_t i, j, xp1, yp1, xp2, yp2, templong;
	char col;
					...
					if (((halfxdim16+xp1) >= 2) && ((halfxdim16+xp1) <= xdim-3))
						if (((midydim16+yp1) >= 2) && ((midydim16+yp1) <= ydim16-3))
						{
							...
							if ((sprite[j].cstat&32) > 0) {
								...
								int32_t fy = mulscale6(tilesizy[sprite[j].picnum], sprite[j].yrepeat);
								int32_t co[4][2], ii;
